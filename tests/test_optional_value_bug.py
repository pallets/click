...
class TestOptionalValueBug:
    ...
    def test_basic_optional_value(self):
        ...
        assert result.exit_code == 0, (
            f"Got exit code {result.exit_code}: {result.output}"
        )
    ...
    def test_multiple_optional_values(self):
        ...
        @click.command()
        @click.option(
            "--verbose", "-v", is_flag=False, flag_value="INFO", default="WARNING"
        )
        @click.option(
            "--config", "-c", is_flag=False, flag_value="default.cfg", default=None
        )
...
def test_simple_reproduction():
    ...
    assert result.exit_code == 0, (
        "Bug confirmed: option with flag_value requires argument"
    )
