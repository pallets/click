    ...
    * ``reset`` (reset the color code only)
    ...
    if fg:
        try:
            ...
            raise TypeError("Unknown color {colour!r}".format(colour=fg)) from None  # noqa: UP032

    if bg:
        try:
            bits.append(f"\033[{_interpret_color(bg, 10)}m")
        except KeyError:
            raise TypeError("Unknown color {colour!r}".format(colour=bg)) from None  # noqa: UP032
