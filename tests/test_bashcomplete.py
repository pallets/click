...
def test_long_chain():
    ...
    assert choices_without_help(cli, ['asub', 'bsub', 'csub', '--csub-opt'], '') == [c + ' ' for c in CSUB_OPT_CHOICES]
    assert choices_without_help(cli, ['asub', 'bsub', 'csub'], '--csub') == ['--csub-opt ', '--csub ']
    assert choices_without_help(cli, ['asub', 'bsub', 'csub', '--csub'], '') == [c + ' ' for c in CSUB_CHOICES]
    assert choices_without_help(cli, ['asub', 'bsub', 'csub', '--csub-opt'], 'f') == ['foo ']
    assert choices_without_help(cli, ['asub', 'bsub', 'csub'], '') == [c + ' ' for c in COLORS]
...
def test_absolute_path():
    ...
    with tempdir() as temp:
        ...
        assert list(get_choices(cli, "lol", ["--path"], temp)) == [
            (temp + os.path.sep, None)
        ]
        assert list(get_choices(cli, "lol", ["--path"], temp + os.path.sep)) == [
            (os.path.join(temp, "dira") + os.path.sep, None),
            (os.path.join(temp, "dirb") + os.path.sep, None),
        ]
        assert list(
            get_choices(cli, "lol", ["--path"], os.path.join(temp, "dirb"))
        ) == [(os.path.join(temp, "dirb") + os.path.sep, None)]
        assert list(
            get_choices(
                cli, "lol", ["--path"], os.path.join(temp, "dirb") + os.path.sep
            )
        ) == [(os.path.join(temp, "dirb", "file.txt") + " ", None)]
...
def test_relative_path():
    ...
    with tempdir(chdir=True) as temp:
        ...
        assert list(get_choices(cli, "lol", ["--path"], "")) == [
        ...
        assert (
            list(get_choices(cli, "lol", ["--path"], os.path.join("x", "y", "z"))) == []
        )
        assert list(get_choices(cli, "lol", ["--path"], "dira")) == [
            ("dira" + os.path.sep, None)
        ]
...
def test_files_only():
    ...
    with tempdir(chdir=True) as temp:
        ...
        assert list(get_choices(cli, "lol", ["--path"], "fil")) == [("file.txt ", None)]
        assert list(get_choices(cli, "lol", ["--path"], "dir")) == [
            ("dira" + os.path.sep, None),
            ("dirb" + os.path.sep, None),
        ]
